...
function getTable(prefix, status, records) {
  const header = [
    ...
    {key: 'issues', title: '결함링크', width: '30%'},
  ]
...
function getDelayTable(records) {
  ...
  records.length &&
    records.map(aRecord => {
      ...
      const statusColor = dayNum > statusRedline ? 'red' : dayNum > statusYellowline ? 'yellow' : 'aqua'
      const statusTag = isUpgrade ? `<span class='issue-clear'>${MARK_INIT}</span>` : `<span class='status' style='background-color: ${statusColor}'></span>`
      const status = `
      <td style='text-align: center; word-break: break-all;'>
        <div class='td-block'>
          <div class='td-edit'>
